...
#define PARITY_ADR      0XF930    // Parity biy (0:none, 1:even, 2:odd)
//
...
#define TIMEOUT 200      // tiempo hasta dar por fallida la petición
#define POLLING 2000     // tiempo entre toma de datos, no se usa, solo una petición por parámetro.
...
void setup() {
  ...
  if (!processRequest(parameterPacket)) return;
  ...
  if (!processRequest(parameterPacket)) return;
  Serial.print(F("Time of display in turns (0 - 30 seconds): ")); Serial.print(parameter.Array[0], HEX);
